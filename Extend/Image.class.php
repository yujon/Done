<?php
 //图像处理类，可以完成对图像进行缩放和加图片水印的操作。     
	...
	class Image {
		protected $path;   //图片所在的目录

		
		/**
		 * 创建图像对象时传递待处理图像的存放目录，默认值是框架的文件上传目录
		 * @param	string	$path	可以指定处理图片的目录
		 */
		function __construct($path=""){
			if($path==""){
				$path=APP_PUBLIC_PATH."uploads";
			}
			$this->path=$path;
		}
		...
		function thumb($name,$width, $height,$qz="th_"){
			...
			$size=$this->getNewSize($name,$imgInfo,$width,$height,$half);       //获取新图片尺寸
			$newImg=$this->kidOfImage($srcImg,$imgInfo,$size);   //获取新的图片资源
		...
		*/ 
		function waterMark($groundName, $waterName, $waterPos=0, $qz="wa_"){
			...
			if(file_exists($curpath.$groundName) && file_exists($wpath.$waterName)){
				...
				$groundImg=$this->copyWaterImg($groundImg, $waterImg, $waterInfo,$pos);  //拷贝图像
		...
		function copyImage($srcName,$desName,$desPath=""){
			$srcImage = $this->path."/".$srcName;
			if($desPath==""){
				$desPath = $this->path;
			}
			$desImage = $this->path."/".$desName;			
			$res = @copy($srcImage,$desImage);
			if(!$res){
				return false;
			}
			return true;
		...
		function moveImage($srcName,$desName,$desPath=""){
			$srcImage = $this->path."/".$srcName;
			if($desPath==""){
				$desPath = $this->path;
			}
		...
		private function getNewSize($name,$imgInfo,$width,$height){	
		...
		private function kidOfImage($srcImg,$imgInfo,$size){
		...
		private function copyWaterImg($groundImg, $waterImg,$waterInfo , $pos){
